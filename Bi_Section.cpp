...
// Bi section method to find a root in the interval [a, b]
double bisection(vector<double>&a,double left, double right) {
    double c;
    int iterations = 0;
...
void findMultipleRoots(vector<double>&a,double start, double end) {
    set<double> roots;
    double intervalSize = 0.0001; // Size of the interval to search for roots
    ...
    for (double i = start; i <= end; i += intervalSize) {
        ...
        if (f(a,left) * f(a,right) < 0) {
            double root = bisection(a,left, right);
